...
During `/speckit.implement`, Context Pinning restricts what can be read:
...
| Part | What | When |
|------|------|------|
| **A. Wire Stations into Planning** | Read stations/subagents before designing | During `/speckit.plan` |
| **B. Self-Contained Tasks** | Embed ALL context INTO task files | During `/speckit.tasks` |
...
Task files generated by `/speckit.tasks` MUST be self-contained:
