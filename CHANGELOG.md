...
- **`registry.testing.enabled` field (v0.5+)** — new primary field in `templates/registry-template.yaml`'s existing `testing:` block. Values: `null | true | false | "later"`. Controls whether Phase 0.85 runs. Provenance tracked in `_provenance.testing.enabled` (`human | accepted_recommendation | migrated_v0.5`). Value coercion is enforced at the gate — string `"yes"`/`"true"`/`"1"` triggers a hard warning and the value is coerced to null (which then routes through Phase 0.85 branches #4/#5 by session mode — interactive re-prompt vs non-interactive silent skip). The 6 existing testing fields (`unit_framework` etc.) become **derivation outputs** — populated by Phase 0.85 from the tech stack, never asked at /clarify.
- **`registry.lifecycle.intent` field (v0.5+)** — new field extending the existing `lifecycle:` block. Values: `production | prototype | throwaway | null`. When `production`, `suppress_when` anti-keywords are bypassed unconditionally — a "spike" or "prototype" keyword in a production spec cannot silently suppress the testing question. Framework-level setting, no HITL gate required.
- **`suppress_when` trigger-schema field (v0.5+)** — new optional predicate on triggers in `.specify/knowledge/triggers.yaml`. Structure: `{ anti_keywords: [...], lifecycle_override: production }`. Semantics: soft-warning + `AskUserQuestion` (never silent skip) when the spec matches any anti-keyword AND `lifecycle.intent != production`. Introduced to close the "payment-spike" footgun. Documented in the file's header comment block.
- **`testing.enabled` architectural lurker (v0.5+)** — added to `web_with_api`, `mobile`, and `desktop` packs in `.specify/knowledge/architectural-lurkers.yaml`. `tier: must` (fires in Lite mode, displaces one existing pack must in priority order — accepted trade). Rationale interpolates per-app-type ("Web apps ship to real users — regression cost outpaces test authoring cost within 2 sprints" / "App store review cycles amplify regression cost" / "Desktop update rollouts are slow — bugs live in the wild for weeks"). The `library:` pack is unchanged (libraries already require `testing.unit_framework` since v0.2).
- **`testing` trigger (v0.5+)** — new keyword-fired trigger in `.specify/knowledge/triggers.yaml`. Fires on `test`, `TDD`, `coverage`, `regression`, `QA`, `CI`, `flake`, `quality gate`, `reliability SLO`, `P0 bug`. Backs up the architectural-lurker path for library projects and any spec whose text signals testing concern.
- **Phase 0 Testing-Consent Migration hook (v0.5+)** — added to `templates/commands/plan.md` alongside the existing `interview_completed` genesis check. Fires ONLY when `testing.enabled` key is absent (v0.4-shaped registry). Interactive session: one-time `AskUserQuestion` with Recommended: Yes, writes `_provenance: migrated_v0.5` on Yes / `_provenance: human` + `testing.enabled: false` on No. On No: never re-asks (key presence, even at `false`, suppresses re-prompt). Non-interactive session: skips silently with logged reason. Placement is deliberately at Phase 0 (not Phase 0.5) — Phase 0.5 already fires 4 `AskUserQuestion` calls; stacking a 5th mid-flow risks question-collision UX regression.
- **Phase 0.85 — Test Strategy Derivation (HITL #3.5, v0.5+)** — new /plan sub-phase in `templates/commands/plan.md` between Phase 0.8 (Frontend/UI) and Phase 0.9 (Registry Sync). Six-branch consent gate: `enabled: true` + human/accepted_recommendation → derive; `enabled: true` + migrated_v0.5 or absent-provenance → inline confirm; `enabled: null` interactive → inline consent ask; `enabled: null` non-interactive → skip logged; `enabled: false | "later"` → skip silent. Derivation reads tech-stack fields already pinned via Phase 0.5-0.8, matches testing subagents by semantic keyword, consults Station 12 test pyramid, emits a 7-column Test Strategy table into `plan.md`. HITL variant copies Phase 0.7's per-warning pattern — one `AskUserQuestion` per Test Strategy row (options: Accept / Change tool / Change coverage / Drop this layer). Never coarse approve/reject.
- **`## Test Strategy` section in `templates/plan-template.md`** — placed between Tech Stack Validation and Frontend/UI Specifications sections. 7-col table (`Layer | Test Type | Tool | Coverage | Task-Range Slot | Subagent / Source | Rationale`), consent-status header, approval fields matching Tech Stack Approval style.
- **Test Strategy Slice extraction + embedding in `templates/commands/tasks.md`** — new Step 1c loads plan.md's `## Test Strategy` for test-writing tasks (Task Target ∈ {unit_test, integration_test, e2e_test}). New `### Test Strategy Slice` block in the task-file template embeds the matched row (Layer, Test Type, Tool, Coverage, Subagent, Rationale) — implementer respects D3 Context Pinning without ever reading plan.md.
- **12 testing subagents** imported from user's local library into `.specify/subagents/testing/`:
  - **8 SAFE** (frontmatter normalization only): `python/pytest-patterns.md` (backend), `kotlin/kotest-mockk.md` (mobile), `rust/cargo-testing.md` (universal), `go/go-testing.md` (backend), `cpp/googletest.md` (universal), `python/django-tdd.md` (backend), `java/springboot-tdd.md` (backend), `php/laravel-tdd.md` (backend).
  - **4 NEEDS-EDIT** (content sanitized on import): `web/playwright-webapp.md` ([web, backend] — /tmp paths + ❌/✅ emoji + broken helper-script refs stripped), `web/playwright-e2e.md` ([web, backend] — companion-runtime sections + `plugin:` YAML block + `📸` emoji + demo creds stripped), `android/adb-ui-verification.md` (mobile — 🛠🚀💡 emojis stripped), `cross-cutting/k6-load.md` (universal — typos fixed + cross-refs + demo creds stripped).
  - **New `## Testing` category in `.specify/subagents/_index.md`** — registers all 12 with `| Agent | File | Platform | Use When |` table + Folder Structure tree entry.
- **`docs/testing-derivation.md`** (new) — user-facing guide: how to opt in, what the Test Strategy table means, how to override derived defaults per row, roadmap for MANUAL-CONFIGURE stacks (iOS, desktop, web frontend JS/TS unit, accessibility).

### Changed

- **Directive 8 (Self-Contained Tasks) — companion row** added to the Embedded Context table: *"Test Strategy Slice (v0.5+) — Extracted from `plan.md` `## Test Strategy` table by `/tasks`, sliced per task. Required for test-writing tasks (Task Target ∈ {unit_test, integration_test, e2e_test}); optional embed for feature tasks with test-adjacency."* Row lands immediately below the v0.4 Efficiency Tier Hint row.
- **Directive 6 (HITL Checkpoints) — extended-scope rationale clarification** added to the directive body. One paragraph: *"Extended scope (v0.5+): Derivation checkpoints for tech-stack subdomains — testing (Phase 0.85), observability (Phase 0.86, reserved), etc. — inherit this directive's HITL contract. They do not replace or expand the 4-checkpoint floor; they compose alongside it under the same rules."* Not a directive amendment — a clarification for discoverability. No renumbering.
...
### Honest disclosures (v0.5.0)

- **Consent is a hard gate.** Zero derivation tokens spent without `testing.enabled: true` recorded in the registry with a valid provenance (`human`, `accepted_recommendation`, or `migrated_v0.5` after confirm). Users who say No are never re-asked; the key presence at `false` suppresses re-prompt on every subsequent run.
- **Subagent coverage is incomplete on purpose.** Twelve testing subagents ship with v0.5 — mostly backend + mobile + language-native. **Stacks without derived support fall back to `MANUAL-CONFIGURE`** (visible marker in the Test Strategy table, never silent): native iOS (XCTest), Electron/Tauri desktop, web frontend JS/TS unit (Vitest/Jest/RTL — Playwright is covered), web accessibility (axe/screen-reader), API security, Ruby/Rails, Node/TS backend unit (Playwright-only), .NET/C#, Vue/Svelte/Angular frontend unit, contract testing, visual regression, mutation testing. Users on those stacks see the fallback marker and configure manually. Iterated in v0.6+.
- **The `library` pack is intentionally unchanged.** Libraries have required `testing.unit_framework` since v0.2 (line 288 of `architectural-lurkers.yaml`); no new testing.enabled question there. If a library user answers Yes to that existing prompt, Phase 0.85 still derives — just with `unit_framework` pre-pinned instead of derived.
- **`suppress_when` is soft-warning + confirm, never silent skip.** A spec containing "spike" or "prototype" triggers `AskUserQuestion` ("This spec mentions 'spike' — skip testing setup? [Y/N]") rather than silently omitting the question. `lifecycle.intent: production` bypasses the suppression check unconditionally, closing the "payment-spike" footgun.
- **Reserved task-range slots may collide.** Cross-cutting tests target T-080-089 and E2E tests target T-090-099 (per `tasks.md` reservations at lines 146-147). If a feature already uses those slots for non-test work, `/atomicspec.tasks` probes for collision and falls back to append-at-tail — no overwrite.
- **No new Directive.** v0.5 amends D8 in-place (new row) and clarifies D6 (extended-scope note). Nine directives stays nine. Follows the v0.2 / v0.4 in-place-amendment precedent.
...
1. Existing `registry.yaml` files without `testing.enabled` key continue to work. First `/atomicspec.plan` run under v0.5 fires the Phase 0 Testing-Consent Migration hook exactly once, with Recommended: Yes and app-specific rationale.
2. On **Yes**: writes `testing.enabled: true` with `_provenance: migrated_v0.5`. Phase 0.85 fires branch #2 (inline confirm-migration) to upgrade provenance to `human` before deriving.
3. On **No**: writes `testing.enabled: false` with `_provenance: human`. Phase 0.85 skips silently on every subsequent run. Never re-asks. Log entry appears in `specs/_defaults/changelog.md`.
4. Non-interactive session (`--no-review` or no TTY): skips migration silently. Phase 0.85 later hits branch #5 (null in non-interactive) and skips derivation. No infinite loop, no silent burn.
5. Existing `testing.*` fields (v0.4 shape) with non-null values: Phase 0.85 treats registry as `enabled: true` with `_provenance: migrated_v0.5`, confirms via HITL before deriving remaining fields. Pinned values win.
6. Contract test: v0.5-migrated consumer that answered No produces byte-identical v0.4.1 output on subsequent runs.

Consumers wanting to opt in immediately without waiting for the migration prompt:
1. Edit `specs/_defaults/registry.yaml`: set `testing.enabled: true` and `_provenance.testing.enabled: human`.
2. Run `/atomicspec.plan` — Phase 0.85 fires branch #1 directly.

## [0.4.0] - 2026-08-21

> **v0.4 theme**: introduce advisory model-tier routing so consumer projects can split coordinator-role turns (Phase 0 orientation, gate checks, stamp writes) from implementer-role turns (code generation, verification loop) and HITL turns (Directive 6 checkpoints) onto different model tiers — Haiku / Sonnet / Opus by default on Anthropic, arbitrary strings for other providers. Shipped **advisor-off by default**: consumer projects on v0.3 see byte-for-byte identical behavior after upgrade. Per-feature measurement primitive lands in v0.4.1; v0.4 ships the config surface + lightweight advisory reporting so the tier map is exercisable and the promotion evidence bar is buildable. Follows the v0.3.0 "surface-first, enforcement-second" pattern for Directive 9 — with an honest correction: v0.3.1 was never released, so the runtime gate (`check-prerequisites --check-orientation`) originally scoped there now targets v0.4.1 alongside `baseline record`.
