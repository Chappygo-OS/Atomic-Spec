...
Task files generated by `/speckit.tasks` MUST be **self-contained**. During implementation, Context Pinning (Directive 3) prevents reading plan.md, spec.md, stations, and subagents. Therefore, ALL context must be embedded INTO each task file.
...
**Rationale**: Subagents during `/speckit.implement` are "blind" to stations, subagents, plan.md, and spec.md. Embedding context ensures they follow project patterns instead of guessing.
