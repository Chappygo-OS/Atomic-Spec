...
**New in v0.5 — testing derivation.** Every `/atomicspec.clarify` session for a web / mobile / desktop feature now asks one yes/no question: *"Do you want automated tests for this feature?"* — with app-specific rationale explaining why tests would save time. If yes, a new `/atomicspec.plan` sub-phase (**Phase 0.85 — Test Strategy Derivation, HITL #3.5**) reads the pinned tech stack, matches testing subagents by semantic keyword, and emits a 7-column Test Strategy table for row-by-row HITL approval. `/atomicspec.tasks` then slices the strategy per test-writing T-XXX file and embeds it under Directive 8 — so the implementer respects Context Pinning (Directive 3) without ever reading `plan.md`. **Zero derivation tokens spent without recorded consent.** Consumers who answer No are never re-asked. Ships with **12 testing subagents** at `.specify/subagents/testing/` (Python/pytest, Django, Kotlin+Kotest, Rust cargo test, Go, C++ GoogleTest, Java Spring Boot, PHP Laravel, Playwright x2, Android ADB verification, k6 load); other stacks fall to a visible `MANUAL-CONFIGURE` marker (never silent). Directive 8 gains a companion "Test Strategy Slice" row; Directive 6 gets a one-sentence extended-scope clarification (the 4-checkpoint floor stays). Full details: [`docs/testing-derivation.md`](./docs/testing-derivation.md).
