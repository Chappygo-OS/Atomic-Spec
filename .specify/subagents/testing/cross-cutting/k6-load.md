...
model: opus
category: testing/cross-cutting
---
...
export default function () {
  ...
  const postRes = http.post('https://api.example.com/users', 
    JSON.stringify({ name: 'Test User', email: 'user@your-domain.example' }),
    {
      headers: {
...
export default function () {
  ...
  const loginRes = http.post('https://api.example.com/login', 
    JSON.stringify({ email: 'user@your-domain.example', password: 'REPLACE_WITH_TEST_PASSWORD' })
  );
...
- `(perf specialist)` - For broader performance optimization
- `(API mocking specialist)` - For API mocking during testing
- `(app perf specialist)` - For performance optimization
