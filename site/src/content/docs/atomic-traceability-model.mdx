...
6. **Human-In-The-Loop** — `/atomicspec.plan` pauses at Phase 0.5 (Tech Stack Review) as the constitutionally required checkpoint; `plan-template.md` additionally enforces template-driven pauses at Phase 0.7 (Validation), Phase 0.8 (UI), and Phase 0.9 (Registry Sync). v0.5+ adds an optional derivation checkpoint at Phase 0.85 (Test Strategy Derivation) that inherits the D6 contract — composes alongside the 4-floor, never replaces it
7. **Project Defaults Registry** — all commands read `specs/_defaults/registry.yaml` and enforce project-wide standards
8. **Self-Contained Tasks** — task files embed all context (registry values, domain rules, gate criteria, and — v0.5+ — the Test Strategy Slice for test-writing tasks) needed for execution under Context Pinning
...
Article IX's invariance is the entire value proposition. Across time, across projects, across AI models, the nine directives stay constant — so a task file generated by one agent on Tuesday can be implemented by a different agent on Friday and still produce compatible, traceable work. Articles I–VIII can evolve per project; Article IX is invariant within a major version. (Directive 9 was added in the v0.3 minor bump as a sibling to Directive 3, not an expansion — so Directives 1–8 remain verbatim since v0.1.)
